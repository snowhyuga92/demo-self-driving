...
def main():
    ...
    readme_text = st.markdown(
        get_file_content_as_string("instructions.md")
    )
    ...
    if app_mode == "Show instructions":
        ...
        readme_text.empty()
        st.code(
            get_file_content_as_string("streamlit_app.py")
        )
...
def run_the_app():
    ...
    selected_frame_index, selected_frame = (
        frame_selector_ui(summary)
    )

    if selected_frame_index is None:

        st.error(
            "No frames fit the criteria. "
            "Please select different label or number."
        )
...
def frame_selector_ui(summary):
    ...
    selected_frames = get_selected_frames(
        summary,
        object_type,
        min_elts,
        max_elts,
...
def get_selected_frames(
    ...
    return summary[
        np.logical_and(
            summary[label] >= min_elts,
            summary[label] <= max_elts,
        )
    ].index
...
def get_file_content_as_string(path):
    ...
    url = (
        ...
        "streamlit/demo-self-driving/master/"
        + path
    )
